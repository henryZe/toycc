			      ...
			      const struct VarAttr *attr)
{
	...
	const char *name_str = get_ident(ty->name);
	struct Obj *fn = find_func(name_str);

	if (fn) {
		// Redeclaration
		if (!fn->is_function)
			error_tok(tok, "re-declared as a different kind of symbol");
	...
	// if it's declaration, return
	if (consume(&tok, tok, ";"))
		return tok;
	// else it's definition, continue
