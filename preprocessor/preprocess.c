					...
					const char *va_args_name)
{
	...
	if (va_args_name) {
		...
		if (equal(tok, ")")) {
			arg = calloc(1, sizeof(struct MacroArg));
			arg->tok = new_eof(tok);
...
//   ...
//   #endif
static const char *detect_include_guard(struct Token *tok)
{
	...
	const char *macro = strndup(tok->loc, tok->len);
	tok = tok->next;

	if (!is_hash(tok) ||
	    !equal(tok->next, "define") ||
				  ...
				  struct Token *filename_tok)
{
	...
	static struct HashMap include_guards;
	const char *guard_name = hashmap_get(&include_guards, path);
	if (guard_name && hashmap_get(&macros, (void *)guard_name))
		return tok;
	...
	guard_name = detect_include_guard(tok_header);
	if (guard_name)
		hashmap_put(&include_guards, path, (void *)guard_name);
